...
// Usage:
// [cALL] = CurveMultiExt_withFund(abs(tfr).', tfrtic, c_fund, lambda, mu*fs, h, round(BW/basicTF.fr))
// tfr: size of [Frequency] by [Times] matrix
// tfrtic: frequency tic of tfr
// c_fund: the informed fundamental IF
// lambda: Smooth penalty
// mu: similarity penalty
// h: the h-th multiple IF (integer)
// BW: searching bandwidth (in Hertz)

function [f_est,modos_est,b_e] = SAMD(f,A,phi,D,order_Phi,phiALL)
...
modelfun = @(b,X)(regresion(b,X,D,phiALL));
...
%     alpha_inic(1,:) = 2:D(i);
%     valor_inicial = [valor_inicial alpha_inic(:)']; % 2sum(D(i))+Order_Phi*(D(i)-1)
% end
options = statset;
...
function v = regresion(b,X,D,phiALL)
I = length(D);
A = X(1:I,:);
phi = X(I+1:end,:);
v = A(1,:).*(b(1:D(1))*cos((phiALL)) + b(D(1)+1:2*D(1))*sin(phiALL));
for i = 2:I
    v = v + A(i,:).*(b(2*sum(D(1:i-1))+1:2*sum(D(1:i-1))+D(i))*cos(phiALL) + ...
        b(2*sum(D(1:i-1))+D(i)+1:2*sum(D(1:i)))*sin(phiALL));
end  
end
